  ...
  return (
    ...
    <nav className='bg-neutral-900 h-auto items-center sm:h-auto flex justify-center mt-4 mb-1 sm:mt5 sm:mb-2 md:mt-6 md:mb-3 lg:mt-6 lg:mb-4 w-11/12 md:w-4/5  m-auto rounded-2xl shadow-lg shadow-white/50 '>
      ...
      <div id="modal" className='fixed z-10 w-full h-screen top-0 left-0 backdrop-blur-sm hidden fading-in' onClick={() => hideModal()} >
        ...
        {navLinks?.map((navLinks) => (
            <li key={navLinks.key+'modal'} className='mx-5 hover:text-white p-1 transition text-xl my-2' onClick={() => hideModal()}><a href={navLinks.link} onClick={() => setShowModalNav(!ShowModalNav)}>{navLinks.title}</a></li>
            ))}
          </ul>
        </div> 
      </div>
